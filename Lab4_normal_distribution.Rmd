...
In this lab, you'll investigate the probability distribution that is most central to statistics: the normal distribution. If you are
confident that your data are nearly normal, that opens the door to many powerful statistical methods. Here we'll use the graphical tools of R to assess the normality of our data and also learn how to generate random numbers from a normal distribution.
...
```{r}
library(gridExtra)
mcd <- ggplot(mcdonalds, aes(x=cal_fat)) +
  geom_histogram(bins=10) +
  labs(title="McDonalds")
dq <- ggplot(dairy_queen, aes(x=cal_fat)) +
  geom_histogram(bins=10) +
  labs(title="Dairy Queen")

grid.arrange(mcd, dq, ncol=2)
```

*The centers are comparable, in the 150-350 calories from fat range, the shapes are comparable as well, but their spreads are not. McDonalds has high calories from fat items that range past 1000, while Dairy Queen's items top out around 700.*

## The normal distribution

In your description of the distributions, did you use words like
*bell-shaped*or *normal*? It's tempting to say so when faced with a unimodal symmetric distribution.
...
*The plot appears to be diagonal, appearing more normally distributed than the real data*
...
*The simulated plots look simliar to the normal probability plot for the calories from fat, providing evidence that the calories are nearly normal*
...
*The simulated plots do not have the same skew that the McDonalds distribution shows, which supports that the items do come from a normal distribution*
...
distribution. Once you decide that a random variable is approximately normal, you can answer all sorts of questions about that variable related to probability. Take, for example, the question of, "What is the probability that a randomly chosen Dairy Queen product has more than 600 calories from fat?"
...
*1: What is the probability that a Subway item has cholesterol over 180? 2: What is the probability that a Taco Bell item has cholesterol below 55?*
...
submean<- mean(subway$cholesterol)
subsd   <- sd(subway$cholesterol)
tbmean <- mean(taco_bell$cholesterol)
tbsd <- sd(taco_bell$cholesterol)

# normal probability, Subway
pnorm(q = 180, mean = submean, sd = subsd, lower.tail=FALSE)
# empirical probability
subway %>% 
  filter(cholesterol > 180) %>%
  summarise(percent = n() / nrow(subway))
```

*The normal probability of a Subway item having cholesterol over 180 is nearly 5x lower than the empirical probability.*

```{r}
# normal probability, Taco Bell
pnorm(q = 55, mean = tbmean, sd = tbsd)
# empirical probability
taco_bell %>% 
  filter(cholesterol < 55) %>%
  summarise(percent = n() / nrow(taco_bell))
```

*The normal probability of a Taco Bell item having a cholesterol below 55 is much closer to it's empirical probability.*
...
*Dairy Queen, Arby's, and Chick-Fil-A appear closest to normal for sodium*

8.  Note that some of the normal probability plots for sodium
distributions seem to have a stepwise pattern. why do you think this might be the case?

*Sodium content can vary amongst different food items, for example a main dish versus a side dish or salad*
...
cfmean<- mean(chick$total_carb)
cfsd   <- sd(chick$total_carb)

ggplot(data = chick, aes(sample = total_carb)) + 
  geom_line(stat = "qq")
```

*This distribution looks right-skewed, let's take a look at a histogram*

```{r}
ggplot(data = chick, aes(x = total_carb)) +
        geom_blank() +
        geom_histogram(aes(y = ..density..)) +
        stat_function(fun = dnorm, args = c(mean = cfmean, sd = cfsd), col = "red")
```

*The distribution in the histogram appears somewhat left-skewed.*
