...
As this is survey, the study is observational. The study design cannot
answer the question whether beauty leads to differences in course
evaluations. A more appropriate question would be, is there a
relationship between beauty and course evaluations?
...
The distribution is left-skewed -- this makes sense if you believe that
most students give good scores to their teachers than bad.
...
```{r}
plot(cls_perc_eval ~ bty_avg, evals)
```

There doesn't appear to be a linear relationship between beauty average
and the percentage of students who submit evaluations.
...
There appear to be fewer points on the scatterplot than observations in
the dataframe.
...
Equation $\hat{y} = 3.88034 + 0.06664$

The p-value is much lower than alpha (0.05) so the relationship is
significant, but the slope is very slight and the adjusted R-squared
indicates the beauty average only accounts for 33% of the variance in
score.
...
The three conditions for least squares regression are linearity,
constant variability, and nearly normal distribution.

The normal Q-Q plot does indicate a somewhat linear relationship, but
the residuals are not uniformly distributed, and the leverage plot does
not indicate nearly normal distribution.
...
The residuals are not uniformly distributed. The residuals are nearly
normally distributed with a left skew. The residuals do appear to be
linearly related.
...
Yes, the addition of gender to the model has changed the parameter
estimate. bty_avg is still a significant predictor of score, but adding
gender to the model, which is significant based on its p-value,
increases the significance of bty_avg but decreases the adjusted
R-squared to account for just 5% of the variance of score.
...
$$\hat{y}(score) = 3.74734 + (0.07416 \times btyavg) + 0.17239 \times (1)$$
\<- Males
...
```{r}
m_bty_gen <- lm(score ~ bty_avg + rank, evals)
summary(m_bty_gen)
```

R recodes the rank variable twice.
...
Number of professors should have no bearing on professor score.
...
cls_profssingle has the highest p-value (0.77806), confirming my
suspicion.

13. Interpret the coefficient associated with the ethnicity variable.

Professors that are not minorities have scores 0.1234929 higher than
minorities.

14. Drop the variable with the highest p-value and re-fit the model.

```{r}
m_full.2 <- lm(score ~ rank + gender + ethnicity + language + age + cls_perc_eval 
...
change? (One of the things that makes multiple regression interesting is
that coefficient estimates depend on the other variables that are
included in the model.) If not, what does this say about whether or not
the dropped variable was collinear with the other explanatory variables?

```{r warning=FALSE, message=FALSE}
coeffs1 <-m_full$coefficients
coeffs2 <-m_full.2$coefficients

rbind(coeffs1, coeffs2)
```

Most of the variables fluctuated only slightly, with the exception of
color pictures which changed by over 4, indicating the variables depend
...
```{r}
m_full.3 <- lm(score ~ ethnicity + gender + language + age + cls_perc_eval + cls_credits + bty_avg + pic_color, evals)

summary(m_full.3)
```

$$\hat{y} (score) = 3.771922 + 0.167872 \times ethnicitynot minority(1) + 0.207112 \times gendermale(1) -0.206178 \times languagenon-english(1) -0.006046 \times age + 0.004656 \times cls-perc-eval + 0.505306 \times cls-creditsone-credit + 0.051069 \times bty-avg - 0.190579 pic-colorcolor(1)$$
16. Verify that the conditions for this model are reasonable using
    diagnostic plots.
    
```{r}
ggplot(data = m_full.3, aes(x = .resid)) +
  geom_histogram(binwidth = 0.25) +
  xlab("Residuals")
```
...
The residuals are still left-skewed and non constant variance. However they do appear linearly related.
...
Yes, if certain professors teach more than one course. 
...
Male, non-minority, teaching a one credit class, and having a black and white photograph. 
...
No, in order to generalize this study, we'd need to ensure that other schools have similar populations of professors and students.  
