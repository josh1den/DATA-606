...
*The first histogram uses a binwidth of 30, which groups most of the data near zero, making the differentiation between departure delays unclear. The second histogram reduces the binwidth to 15, making the delays clearer. The third histogram, with a binwidth of 150, makes it nearly impossible to differentiate the delay times.*
...
*The distribution of the arrival delays appears to be normally distributed, with some outliers. Since there are outliers, calculating the median delay is a more appropriate summary statistic than the mean, as the presence of outliers will skew the mean*

```{r solution-3.2}
sfo_feb_flights %>%
  summarise(median_ad = median(arr_delay), min_ad = min(arr_delay), max_ad = max(arr_delay))
```

*We can see the median arrival delay is 11 minutes early, with the minimum 66 minutes early and the maximum 196 minutes late*
...
*United Airlines has the most variable arrival delays*
...
*The median is the overall better choice, as it will not be skewed by any outlier delay events, and though it might be skewed by months with large numbers of delays, that is precisely the information we are after.*
...
*The average speed continues to increase for trips up to 1500 miles, and stays steady for trips of 1500 miles or greater, although there is range of speeds of about 100mph*
...
*It appears that the cutoff for flight delay not affecting an arrival delay is about 80 minutes.*