...
*The distribution of the sample is very close to the distribution of the population*
...
*I wouldn't expect the sample proportion to exactly match another sample but I would expect it to be very close - the samples are taken from the same population*
...
*There are 15000 samples in the distribution, it appears to be normally distributed with a center near 0.2*
...
5.  To make sure you understand how sampling distributions are built,
    ...
    `sample_props_small`. Print the output. 
...
*There's only 21 observations although 25 samples were taken. This is because the observations only represent "Doesn't benefit" observations, and four of the samples did not contain any of those observations, since the sample size was so small*
...
What does each observation in the sampling distribution represent? *the mean of each sample*

How does the mean, standard error, and shape of the sampling distribution change as the sample size increases? *the mean stays roughly the same, but the standard error decreases and the shape becomes more normally distributed around the mean*

How (if at all) do these values change if you increase the number of simulations? *the shape only changes meaningfully if the sample size stays small, otherwise it stays approximately the same*
...
*About 73% of people think the work scientist do benefits their lives*
...
*The sampling distribution appears normally distributed. I would expect the true proportion to be about 80%*
...
*The shape remains approximately normal. I would expect the true proportion to be slightly higher than the smaller sample*
...
*The larger sample had the smaller spread. To make estimates closer to the true value, I would want a sampling distribution with a smaller spread*
